...
export class UpdateProfileComponent implements OnInit {
  ...
  this.http.get(`${API_URL}/profile`,{headers}).subscribe({
      next: (response: any) => {
  ...
  onSubmit(): void {
    ...
    this.http
      .put(`${API_URL}/profile`, { username, bio },{headers})
      .subscribe({
        next: () => {
  ...
  uploadProfilePicture(): void {
    ...
    const token = localStorage.getItem('token')
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`)
  this.http.post(`${API_URL}/profile/picture`, formData,{headers}).subscribe({
      next: () => {
